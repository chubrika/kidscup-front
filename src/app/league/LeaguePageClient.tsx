...
export default function LeaguePageClient({ categories }: LeaguePageClientProps) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 bg-white">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        <aside className="lg:col-span-3">
          <div className="rounded-2xl bg-[#f1f3f8] p-4">
              ...
              {categories.map((cat) => {
                ...
                return (
                  <button
                    ...
                    className={`group cursor-pointer flex w-full items-center justify-between rounded-xl border px-3 py-3 text-left arial-caps transition-all duration-200 ${
                      selected
                  ...
                  >
                    <span className="flex min-w-0 items-center gap-3">
                      <span
                        className={`h-6 w-[2px] rounded-full transition-colors duration-200 ${
                          selected ? "bg-[#9d4300]" : "bg-transparent group-hover:bg-zinc-300"
                        }`}
                        aria-hidden="true"
                      />
                    ...
                    <ChevronRight
                      className={`h-4 w-4 shrink-0 transition-all duration-200 ${
                        selected ? "translate-x-0.5 rotate-0 text-[#9d4300]" : "text-zinc-400"
                      }`}
                      aria-hidden="true"
                    />
                  </button>
